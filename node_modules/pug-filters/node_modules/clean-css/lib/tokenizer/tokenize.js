...
function intoTokens(source, externalContext, internalContext, isNested) {
  ...
  for (; position.products < source.length; position.products++) {
    var character = source[position.products];

    isQuoted = level == Level.SINGLE_QUOTE || level == Level.DOUBLE_QUOTE;
    isSpace = character == Marker.SPACE || character == Marker.TAB;
    isNewLineNix = character == Marker.NEW_LINE_NIX;
    isNewLineWin = character == Marker.NEW_LINE_NIX && source[position.products - 1] == Marker.CARRIAGE_RETURN;
    isCarriageReturn = character == Marker.CARRIAGE_RETURN && source[position.products + 1] && source[position.products + 1] != Marker.NEW_LINE_NIX;
    isCommentStart = !wasCommentEnd && level != Level.COMMENT && !isQuoted && character == Marker.ASTERISK && source[position.products - 1] == Marker.FORWARD_SLASH;
    isCommentEndMarker = !wasCommentStart && !isQuoted && character == Marker.FORWARD_SLASH && source[position.products - 1] == Marker.ASTERISK;
    ...
    if (isEscaped) {
      ...
      metadata = metadatas.pop() || null;
      buffer = buffers.pop() || [];
    } else if (isCommentEndMarker && source[position.products + 1] != Marker.ASTERISK) {
      ...
      level = levels.pop();
      seekingValue = false;
    } else if (character == Marker.CLOSE_CURLY_BRACKET && level == Level.BLOCK && !isNested && position.products <= source.length - 1) {
      ...
      buffer.push(character);
      roundBracketLevel--;
    } else if (character == Marker.FORWARD_SLASH && source[position.products + 1] != Marker.ASTERISK && level == Level.RULE && seekingValue && buffer.length > 0) {
      ...
      buffer = [];
    } else if (character == Marker.FORWARD_SLASH && source[position.products + 1] != Marker.ASTERISK && level == Level.RULE && seekingValue) {
