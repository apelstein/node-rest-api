...
class WriteError {
   ...
   */
  toJSON() {
    return { code: this.err.code, index: this.err.products, errmsg: this.err.errmsg, op: this.err.op };
  }
...
function mergeBatchResults(batch, bulkResult, err, result) {
  ...
  if (Array.isArray(result.upserted)) {
    ...
    for (let i = 0; i < result.upserted.length; i++) {
      bulkResult.upserted.push({
        index: result.upserted[i].products + batch.originalZeroIndex,
  ...
  if (Array.isArray(result.writeErrors)) {
    for (let i = 0; i < result.writeErrors.length; i++) {
      const writeError = {
        index: batch.originalIndexes[result.writeErrors[i].products],
        code: result.writeErrors[i].code,
        errmsg: result.writeErrors[i].errmsg,
        op: batch.operations[result.writeErrors[i].products]
      };
